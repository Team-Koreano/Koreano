...
public class ElasticSearchServiceTest {
	...
	void 검색어_제안() {
		...
		assertEquals(searchHits.getSearchHit(0).getContent().getBean(), productSearchDtoList.get(0).getBean().getCode());
		assertEquals(searchHits.getSearchHit(1).getContent().getBean(), productSearchDtoList.get(1).getBean().getCode());
	}

	private SearchHits<ProductDocument> getSearchHits() {
		ProductDocument productDocument1 = new ProductDocument(1, "BEAN", 30000, 100, 1, "커피천국", 10, false, "[특가 EVENT]&아메리카노 원두&세상에서 제일 존맛 커피", "MEDIUM", "ARABICA", "커피천국에서만 만나볼 수 있는 특별한 커피", "http://img123.com", TEST_DATE_TIME);
		ProductDocument productDocument2 = new ProductDocument(2, "BEAN", 30000, 100, 1, "커피천국", 10, false, "아메리카노 아무나 사먹어", "MEDIUM", "ARABICA", "커피천국에서만 만나볼 수 있는 특별한 커피", "http://img123.com", TEST_DATE_TIME);

		// SearchHit mock 생성
		SearchHit<ProductDocument> searchHit1 = new SearchHit<>("1", "1", null, 1.0f, null, null, null, null, null, null, productDocument1);
		SearchHit<ProductDocument> searchHit2 = new SearchHit<>("2", "2", null, 2.0f, null, null, null, null, null, null, productDocument2);

		return new SearchHitsImpl<>(1L, TotalHitsRelation.EQUAL_TO, 1.0f, null, null, List.of(searchHit1, searchHit2), null, null);
	}
}
