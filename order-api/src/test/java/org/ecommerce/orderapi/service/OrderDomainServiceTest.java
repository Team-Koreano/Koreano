...
//
// 		// when
// 		orderDomainService.placeOrder(userId, request);
//
// 		// then
...
// 		assertEquals(product1TotalPrice, savedOrderItems.get(0).getTotalPrice());
// 		assertEquals(product2TotalPrice, savedOrderItems.get(1).getTotalPrice());
//
...
// 		assertEquals(OPEN, savedOrderItems.get(0).getStatus());
// 		assertEquals(OPEN, savedOrderItems.get(1).getStatus());
// 	}
//
// 	@Test
// 	void 재고부족_주문생성_실패() {
...
// 				() -> orderDomainService.validateStock(productIds, productIdToQuantityMap));
//
// 		// then
...
// 		given(userServiceClient.getUser(anyInt()))
// 				.willReturn(userServiceResponse);
// 		given(orderItemRepository.findOrderItemById(anyLong(), anyInt()))
// 				.willReturn(orderItem);
// 		int previousSize = orderItem.getOrderStatusHistories().size();
//
// 		// when
// 		OrderItemDto orderItemDto = orderDomainService.cancelOrder(userId, orderItemId);
//
// 		// then
// 		verify(orderItem, Mockito.times(1))
// 				.isCancelableStatus();
// 		verify(orderItem, Mockito.times(1))
// 				.isCancellableOrderDate();
// 		verify(orderItem, Mockito.times(1))
// 				.changeStatus(CANCELLED, REFUND);
// 		assertEquals(CANCELLED, orderItemDto.getStatus());
// 		assertEquals(REFUND, orderItemDto.getStatusReason());
...
// 		// when
// 		CustomException exception = assertThrows(CustomException.class,
// 				() -> orderDomainService.validateOrderItem(orderItem));
//
// 		// then
// 		verify(orderItem, Mockito.times(1)).isCancelableStatus();
...
//
// 		// then
// 		verify(orderItem, Mockito.times(1)).isCancellableOrderDate();
